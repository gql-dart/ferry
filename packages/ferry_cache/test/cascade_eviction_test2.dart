...
void main() {
  test('can find nodes which reference an id', () {
    final ids = nodesReferencingId(
      _postData.keys.toSet(),
      (String id) => _postData[id],
      'Post:123',
    );

    expect(ids, {
      PathNodeRef('Query', ['onePost']),
    });
  });
  ...
  test('does not enter endless loop on circular references', () {
    nodesReferencingId(
      _circularPosts.keys.toSet(),
      (String id) => _circularPosts[id],
      'Post:456',
    );
  ...
  test('can find references in lists', () {
    final ids = nodesReferencingId(
      _listPostData.keys.toSet(),
      (String id) => _listPostData[id],
      'Post:123',
    );

    expect(ids, {
      PathNodeRef('Query', ['posts', 0]),
    });
  });

  test('can find references in nested lists which are not normalized', () {
    final ids = nodesReferencingId(
      _listPostDataNested.keys.toSet(),
      (String id) => _listPostDataNested[id],
      'Post:123',
    );
    expect(ids, {
      PathNodeRef('Query', ['posts', 0, 'post']),
    });
  });
  ...
  test('can find references in nested lists which are normalized', () {
    ...
    final ids = nodesReferencingId(
      _listPostDataNestedNormalized.keys.toSet(),
      (String id) => _listPostDataNestedNormalized[id],
      'Post:123',
    );
    expect(ids, {
      PathNodeRef('Query', ['posts', 0, 'post']),
  ...
  test('can find references in double nested lists which are not normalized',
    ...
    final ids = nodesReferencingId(
      _listPostDataDoubleNested.keys.toSet(),
      (String id) => _listPostDataDoubleNested[id],
      'Post:123',
    );
    expect(ids, {
      PathNodeRef('Query', ['posts', 0, 'post', 'post']),
    });
  });
  ...
  test('removes list items in reverse order', () {
    ...
    final ids = nodesReferencingId(
      _listPostDataDoubleRefNested.keys.toSet(),
      (String id) => _listPostDataDoubleRefNested[id],
      'Post:123',
    );
    expect(ids, {
      PathNodeRef('Query', ['posts', 0, 'post']),
      PathNodeRef('Query', ['posts', 1, 'post']),
    });
  });
}
