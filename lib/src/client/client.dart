...
typedef UpdateCacheHandler<T, TVariables extends JsonSerializable>
    = void Function(dynamic proxy, QueryResponse<T> response,
...
class GQLClient {
  ...
  GQLClient(
      {@required this.link,
      @required this.cache,
      this.options = const GQLClientOptions()});

  Stream<QueryResponse<T>> responseStream<T>(QueryRequest<T> requst) {
  ...
  QueryRequest<T> _addTypename<T>(QueryRequest<T> request) {
    ...
    final document =
        transform(request.operation.document, [AddTypenameVisitor()]);
    return request
      ..operation = Operation(
          document: document, operationName: request.operation.operationName);
  }
  ...
  Stream<QueryResponse<T>> _responseStream<T>(QueryRequest<T> queryRequest) {
    final fetchPolicy = queryRequest.fetchPolicy ?? options.defaultFetchPolicy;
    switch (fetchPolicy) {
